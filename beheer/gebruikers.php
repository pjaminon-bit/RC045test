...
function guVindIndex(array $gebruikers, string $naam): ?int {
    foreach ($gebruikers as $i => $g) if (isset($g['gebruikersnaam']) && strcasecmp((string)$g['gebruikersnaam'], $naam) === 0) return $i;
    return null;
}
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    try {
        ...
        if ($actie === 'aanmaken') {
            ...
            elseif (guVindIndex($gebruikers, $naam) !== null) guFlash('Er bestaat al een account met deze gebruikersnaam. Gebruik bij dat account “Wachtwoord wijzigen”.', 'fout');
            elseif (strlen($wachtwoord) < 12) guFlash('Een nieuw wachtwoord moet minimaal 12 tekens lang zijn.', 'fout');
            elseif ($wachtwoord !== $herhaal) guFlash('De twee wachtwoorden komen niet overeen.', 'fout');
            else {
                $gebruikers[] = ['gebruikersnaam'=>$naam,'hash'=>password_hash($wachtwoord,PASSWORD_DEFAULT),'aangemaakt'=>date('c'),'tabs'=>$tabs];
            ...
            $naam=trim((string)($_POST['gebruikersnaam']??'')); $wachtwoord=(string)($_POST['wachtwoord']??''); $herhaal=(string)($_POST['wachtwoord_herhaald']??''); $idx=guVindIndex($gebruikers,$naam);
            if($idx===null) guFlash('Gebruiker niet gevonden.','fout');
            elseif(strlen($wachtwoord)<12) guFlash('Het nieuwe wachtwoord moet minimaal 12 tekens lang zijn.','fout');
            elseif($wachtwoord!==$herhaal) guFlash('De twee wachtwoorden komen niet overeen.','fout');
            else { $gebruikers[$idx]['hash']=password_hash($wachtwoord,PASSWORD_DEFAULT); if(schrijfGebruikers($usersBestand,$gebruikers)){schrijfLog($logBestand,$huidigeGebruiker,'wachtwoord_reset',$naam);guFlash('Wachtwoord van “'.$naam.'” is gewijzigd.');}else guFlash('Wachtwoord kon niet worden opgeslagen.','fout'); }
            ...
            else { array_splice($gebruikers,$idx,1); if(schrijfGebruikers($usersBestand,$gebruikers)){schrijfLog($logBestand,$huidigeGebruiker,'gebruiker_verwijderd',$naam);guFlash('Account “'.$naam.'” is verwijderd.');}else guFlash('Account kon niet worden verwijderd.','fout'); }
...
$laatsteLogins=guLeesLogins($logBestand); $aantalMetGebruikersrecht=0; $aantalLegacy=0;
foreach($gebruikers as $g){$tabs=$g['tabs']??null;if(!is_array($tabs))$aantalLegacy++;if(is_array($tabs)&&in_array('gebruikers',$tabs,true))$aantalMetGebruikersrecht++;}
?><!DOCTYPE html>
...
<form method="post" class="grid2"><input type="hidden" name="csrf" value="<?=guEsc($csrfToken)?>"><input type="hidden" name="actie" value="aanmaken"><div class="velden"><div class="veld"><label>Gebruikersnaam</label><input name="gebruikersnaam" autocomplete="off" required pattern="[A-Za-z0-9._-]{2,30}" placeholder="bijv. secretaris"></div><div class="pwfields"><div class="veld"><label>Wachtwoord</label><div class="pwfield"><input class="pw1" type="password" name="wachtwoord" minlength="12" autocomplete="new-password" required><button class="eye showpw" type="button" aria-label="Wachtwoord tonen" title="Wachtwoord tonen"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.8"/></svg></button></div></div><div class="veld"><label>Herhaal wachtwoord</label><div class="pwfield"><input class="pw2" type="password" name="wachtwoord_herhaald" minlength="12" autocomplete="new-password" required><button class="eye showpw" type="button" aria-label="Wachtwoord tonen" title="Wachtwoord tonen"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.8"/></svg></button></div></div><div class="pwtools"><span class="hint">Minimaal 12 tekens. De generator maakt een willekeurig wachtwoord van 20 tekens.</span><button class="btn secondary genpw" type="button">Genereer sterk wachtwoord</button></div></div></div><div><div class="preset"><button class="btn secondary preset-content" type="button">Contentbeheer</button><button class="btn secondary preset-all" type="button">Alles selecteren</button><button class="btn secondary preset-none" type="button">Alles wissen</button></div><div class="rights"><?php foreach($rechtGroepen as $groep=>$items):?><div class="rechtgroep"><h4><?=guEsc($groep)?></h4><?php foreach($items as $key=>$label):?><label class="check"><input type="checkbox" name="tabs[]" value="<?=guEsc($key)?>" data-recht="<?=guEsc($key)?>"><span><?=guEsc($label)?><?php if(in_array($key,$gevoeligeRechten,true)):?> <small>gevoelig</small><?php endif;?></span></label><?php endforeach;?></div><?php endforeach;?></div></div><div><button class="btn primary" type="submit">Account aanmaken</button></div></form></section>
...
<details><summary>Wachtwoord wijzigen</summary><div class="detailbody"><form method="post" class="velden"><input type="hidden" name="csrf" value="<?=guEsc($csrfToken)?>"><input type="hidden" name="actie" value="wachtwoord"><input type="hidden" name="gebruikersnaam" value="<?=guEsc($naam)?>"><div class="pwfields"><div class="veld"><label>Nieuw wachtwoord voor <?=guEsc($naam)?></label><div class="pwfield"><input class="pw1" type="password" name="wachtwoord" minlength="12" autocomplete="new-password" required><button class="eye showpw" type="button" aria-label="Wachtwoord tonen" title="Wachtwoord tonen"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.8"/></svg></button></div></div><div class="veld"><label>Herhaal nieuw wachtwoord</label><div class="pwfield"><input class="pw2" type="password" name="wachtwoord_herhaald" minlength="12" autocomplete="new-password" required><button class="eye showpw" type="button" aria-label="Wachtwoord tonen" title="Wachtwoord tonen"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.8"/></svg></button></div></div></div><div class="pwtools"><span class="hint">Minimaal 12 tekens.</span><button class="btn secondary genpw" type="button">Genereer sterk wachtwoord</button></div><div><button class="btn primary" type="submit">Wachtwoord wijzigen</button></div></form></div></details>
...
function zetOogStatus(button,zichtbaar){button.setAttribute('aria-label',zichtbaar?'Wachtwoord verbergen':'Wachtwoord tonen');button.setAttribute('title',zichtbaar?'Wachtwoord verbergen':'Wachtwoord tonen');button.innerHTML=zichtbaar?'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3l18 18"/><path d="M10.6 10.7a2.8 2.8 0 0 0 3.8 3.8"/><path d="M9.9 5.2A10.8 10.8 0 0 1 12 5c6 0 9.5 7 9.5 7a15.4 15.4 0 0 1-2.4 3.1"/><path d="M6.1 6.1A15.8 15.8 0 0 0 2.5 12s3.5 7 9.5 7c1.4 0 2.7-.4 3.8-1"/></svg>':'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.8"/></svg>';}
document.addEventListener('click',function(e){const b=e.target.closest('button');if(!b)return;if(b.classList.contains('genpw')){const form=b.closest('form');const p=sterkWachtwoord();form.querySelector('.pw1').value=p;form.querySelector('.pw2').value=p;}if(b.classList.contains('showpw')){const veld=b.closest('.pwfield');const input=veld.querySelector('input');const zichtbaar=input.type==='password';input.type=zichtbaar?'text':'password';zetOogStatus(b,zichtbaar);}if(b.classList.contains('preset-all')||b.classList.contains('preset-none')||b.classList.contains('preset-content')){const form=b.closest('form');form.querySelectorAll('input[name="tabs[]"]').forEach(c=>{if(b.classList.contains('preset-all'))c.checked=true;else if(b.classList.contains('preset-none'))c.checked=false;else c.checked=!['gebruikers','backups','log'].includes(c.value);});}});
