...
$settingsHerstelNodig = $extern && tenantSettingsRuntimeHeeftLeesfout();
$melding = '';
$type = '';
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    if (!csrfOk()) {
        ...
        $type = 'fout';
    } elseif ($settingsHerstelNodig) {
        $melding = tenantSettingsHerstelMelding();
        $type = 'fout';
    } else {
        try {
...
if (isset($_GET['opgeslagen']) && !$settingsHerstelNodig) { $melding = 'Instellingen opgeslagen en direct actief op de website.'; $type = 'ok'; }
if ($settingsHerstelNodig) { $melding = tenantSettingsHerstelMelding(); $type = 'fout'; }
...
<link rel="stylesheet" href="csp205-instellingen-dceb42eadf80.css"><style nonce="<?=siteCspHtmlWaarde(siteCspNonce())?>" id="beheer-preview-theme">#preview{--pv-primary:<?=insEsc($kleuren['primary'])?>;--pv-accent:<?=insEsc($kleuren['accent'])?>;--pv-dark:<?=insEsc($kleuren['dark'])?>;--pv-text:<?=insEsc($kleuren['text'])?>;--pv-bg:<?=insEsc($kleuren['background'])?>;--pv-nav:<?=insEsc($kleuren['nav_background'])?>;--pv-navtext:<?=insEsc($kleuren['nav_text'])?>}</style></head><body>
...
<?php if($settingsHerstelNodig):?><div class="waarschuwing">De instellingeneditor is read-only totdat <code>settings/site.json</code> gecontroleerd is hersteld. De bestaande settingsbytes en brandingassets worden niet door deze pagina gewijzigd.</div><?php endif;?>
...
<button class="btn" type="submit"<?=($extern&&!$settingsHerstelNodig)?'':' disabled'?>>Instellingen opslaan</button>
</div><aside>
<section class="kaart preview" id="preview"><div class="preview-nav"><div class="preview-brand"><?php if($previewLogo!==''):?><img id="preview-logo" src="<?=insEsc($previewLogo)?>" alt=""><?php else:?><span id="preview-logo-placeholder">◆</span><?php endif;?><span id="preview-name"><?=insEsc($naam)?></span></div><div class="preview-links"><span>Over ons</span><span>Contact</span><span>Lid worden</span></div></div><div class="preview-hero"><h3 id="preview-title"><?=insEsc($naam)?></h3><p id="preview-slogan"><?=insEsc($slogan!==''?$slogan:'Jouw vereniging, jouw uitstraling.')?></p><span class="preview-button">Bekijk activiteiten</span></div><div class="preview-body"><div class="preview-card"><strong>Live voorbeeld</strong><br><small>Kleuren en identiteit worden hier direct weergegeven.</small></div></div></section>
