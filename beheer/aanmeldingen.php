        ...
        if($idx===null){aaFlash('Aanmelding niet gevonden.','fout');}
            ...
            if($actie==='accepteren'){
                if($status!=='nieuw'){aaFlash('Alleen een nieuwe aanmelding kan worden geaccepteerd.','fout');}
                    ...
                    $leden=ledenLees();$bestaand=null;
                    foreach($leden['leden'] as $lidIndex=>$lid){if(($lid['aanmelding_id']??'')===$id){$bestaand=$lidIndex;break;}}
                    if($bestaand===null){
                        ...
                        $lid['nummer']=ledenVolgendNummer($leden);$lid['bron']='aanmelding';$lid['aanmelding_id']=$id;
                        $lid['lidmaatschap_type']=(string)($a['lidmaatschap_type']??'');
                        $type=lidmaatschapTypeOpId($lid['lidmaatschap_type']);
                        $lid=ledenZetContributie($lid,(int)date('Y'),[
                            'status'=>'open','bedrag'=>$a['berekend_bedrag']??'',
                            'inschrijfgeld'=>$type['inschrijfgeld']??'','betaald_op'=>'','opmerking'=>'Aangemaakt bij acceptatie van online aanmelding.'
                        ]);
                        $leden['leden'][]=$lid;$leden['volgnummer']=max((int)($leden['volgnummer']??0),(int)$lid['nummer']);
                        if(!ledenSchrijf($leden)){aaFlash('Lid kon niet worden opgeslagen. Aanmelding is niet gewijzigd.','fout');$lid=null;}
