...
c514(str_contains($exec,'function cpeProcessingOpruimen')&&str_contains($exec,"cpeUnlink(\$requestPad,'Verwerkt control-plane queue-item')")&&str_contains($exec,'finally{if($cleanup)cpeProcessingOpruimen($c,$dst,$id);}')&&str_contains($exec,'cpeReconcileProcessing($c)'),'processing-item wordt na definitieve afronding aantoonbaar verwijderd terwijl recoverywaardige state voor reconciliation kan blijven staan');
...
$monitor=(string)file_get_contents($production['monitoring']);$monitorContract=(string)file_get_contents($root.'/app/deployment/monitoring-contract.php');
c514(str_contains($monitor,'$php=PHP_BINARY;')&&str_contains($monitor,'Host-engine PHP CLI is niet exact gepind.')&&str_contains($monitor,"\$checker=\$hostRoot.'/bin/check-vps-health.php';"),'monitoring root-apply draait uitsluitend met exact gepinde host-engine PHP');
c514(str_contains($monitorContract,"'php_binary'=>'/usr/bin/php'.\$php")&&str_contains($monitorContract,"'fpm_service'=>'php'.\$php.'-fpm.service'")&&str_contains($monitorContract,"'php_version'=>\$php"),'monitoringcontract behoudt exacte tenant-PHP en FPM-runtimeidentity');
c514(str_contains($monitorContract,'function monitoring46HostLauncher():string')&&str_contains($monitorContract,"'ExecStart='.\$launcher.' health --monitoring-plan='")&&str_contains($monitorContract,".' --probe --write-status --alert'")&&!str_contains($monitorContract,"'ExecStart='.\$p['runtime']['php_binary']"),'permanente monitoringservice gebruikt uitsluitend de vaste root-owned host-launcher');
