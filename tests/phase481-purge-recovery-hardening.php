...
$recover=strpos($apply,"if(\$actie==='recover-purge')");$recoverBoundary=strpos($apply,'apply48DeleteBoundary($pr,$tb)',$recover);$recoverRm=strpos($apply,'apply48Rm($root)',$recover);c481($recover!==false&&$recoverBoundary!==false&&$recoverRm!==false&&$recoverBoundary<$recoverRm,'recover-purge bewijst deleteboundary vóór recursive delete');
$purge=strpos($apply,"if(\$actie==='purge')");$purgeBoundary=strpos($apply,'apply48DeleteBoundary($p)',$purge);$purgeRm=strpos($apply,'apply48Rm($deleteRoot)',$purge);c481($purge!==false&&$purgeBoundary!==false&&$purgeRm!==false&&$purgeBoundary<$purgeRm,'normale purge bewijst dezelfde deleteboundary vóór recursive delete');
$workflow=(string)file_get_contents($root.'/.github/workflows/deploy-dev.yml');c481(str_contains($workflow,'phase481-purge-recovery-hardening.php'),'fase 4.8.1 purge-hardeningtest draait in CI');
