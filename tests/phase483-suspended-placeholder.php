...
c483($vhost!==''&&str_contains($vhost,'/etc/letsencrypt/live/')&&str_contains($vhost,"['tls']['cert_name']"),'placeholder gebruikt exact het bestaande tenantcertificaat');
...
c483(str_contains($vhost,"style-src \'self\'")&&!str_contains($vhost,"unsafe-inline"),'placeholder-CSP staat uitsluitend eigen stylesheet toe zonder unsafe-inline');
...
c483(str_contains($apply,'href="/style.css"')&&str_contains($apply,"'style'=>'/var/www/verenigingsplatform-suspended/style.css'")&&str_contains($apply,"apply48Write(\$x['style'],apply48SuspendedCss(),0644)"),'placeholder-CSS staat in apart root-owned same-origin bestand');
...
c483(str_contains($activate,'is_link($s[\'enabled\'])')&&str_contains($activate,'@unlink($s[\'enabled\'])')&&str_contains($activate,"['apache']['tenant_https_available']"),'activate schakelt placeholder uit en herstelt normale HTTPS-vhost');
