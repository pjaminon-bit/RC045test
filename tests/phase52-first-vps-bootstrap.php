...
try{
 ...
 $statusPos=strpos($apply,"if(\$mode==='status'){\$state=b52StateRead(\$p,\$ctx['sha256'],true);echo bootstrap52Json(\$state);exit(0);}");$preflightPos=strpos($apply,'b52ProductionPreflight($p,$bins);');$lockPos=strpos($apply,"\$lh=@fopen(\$p['paths']['lock_file'],'c')");c52($statusPos!==false&&$preflightPos!==false&&$lockPos!==false&&$statusPos<$preflightPos&&$preflightPos<$lockPos,'status blijft read-only en volledige production preflight gebeurt vóór het eerste bootstrap-lockbestand');
