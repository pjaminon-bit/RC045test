...
liveFixCheck(str_contains($endpoint,'http_response_code(404)'),'niet-whitelisted/ongeldige content blijft fail-closed met 404');
liveFixCheck(str_contains($endpoint,'$data = [];')&&str_contains($endpoint,'geen fallback naar de')&&str_contains($endpoint,'private tenantopslag blijft de enige bron'),'ontbrekende whitelisted tenantdataset levert lege 200-data zonder legacy fallback');
...
liveFixCheck(
    ...
    && str_contains($isolatie,'lege 200-dataset voor ontbrekende tenantcontent zonder legacy fallback')
    && str_contains($isolatie,"trim(\$outTraversal)==='STATUS=404'"),
    'tenantisolatietest borgt lege whitelisted dataset en houdt traversal op 404'
);
