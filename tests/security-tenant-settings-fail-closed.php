...
try {
    ...
    foreach ($scenario's as $naam => $bytes) {
    ...
    c155(strpos($settingsBron, '$huidig = tenantSettingsLees($basisConfig);') < strpos($settingsBron, '$map = dirname($pad);'), 'settingswrite valideert bestaande state vóór filesystemmutaties');
    ...
    c155(strpos($beheerBron, 'elseif ($settingsHerstelNodig)') < strpos($beheerBron, 'tenantBrandingAssetUpload('), 'beheer blokkeert corrupte state vóór brandingupload');
    ...
    c155(str_contains($brandingBron, 'tenantSettingsLees($config);') && strpos($brandingBron, 'tenantSettingsLees($config);') < strpos($brandingBron, 'atomicFileTxBegin([$root])'), 'brandingtransactie preflight settings vóór snapshot/mutatie');
} finally {
    @chmod($settingsPad, 0640);
    wis155($tmp);
}
