#!/usr/bin/env bash
set -euo pipefail

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$root"

printf '== JavaScript syntax ==\n'
while IFS= read -r -d '' js; do
  node --check "$js"
done < <(find . -type f -name '*.js' \
  -not -path './node_modules/*' \
  -not -path './vendor/*' \
  -not -path './playwright-report/*' \
  -print0)

printf '\n== PHP regressiesuites ==\n'
mapfile -d '' tests < <(find tests -maxdepth 1 -type f -name '*.php' -print0 | sort -z)
if (( ${#tests[@]} == 0 )); then
  echo 'FOUT: geen PHP-regressietests gevonden.' >&2
  exit 1
fi

for test_file in "${tests[@]}"; do
  printf '\n>>> %s\n' "$test_file"
  php "$test_file"
done

printf '\n== #226 operator syntax ==\n'
bash -n ops/vps-test-deploy/finalize-public-root-cutover
bash -n ops/vps-test-deploy/check-public-root-shadow

printf '\n== Apache #226 runtime-regressie ==\n'
bash tests/architecture-226-apache-runtime.sh

printf '\n== Apache + echte PHP-FPM #226 runtime-regressie ==\n'
bash tests/architecture-226-apache-fpm-runtime.sh

printf '\n== Repositorygrenzen ==\n'
test ! -e site-config.local.php

echo "ALLE REGRESSIETESTS GESLAAGD (${#tests[@]} PHP-tests + #226 operator syntax + Apache #226 runtime + echte PHP-FPM/shadow runtime)"
