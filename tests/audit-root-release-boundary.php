...
c157(str_contains($installer,"-m 0444")&&str_contains($installer,'chmod 0555')&&str_contains($installer,'.host-engine-manifest.sha256'),'host-engine installer maakt files read-only, dirs immutable-intent en schrijft manifest');
...
foreach([
    ...
    $cmd='/usr/bin/env bash -n '.escapeshellarg($root.'/'.$script).' 2>&1';exec($cmd,$out,$code);c157($code===0,'bash syntax geldig: '.$script);
}
