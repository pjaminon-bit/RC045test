...
laa(str_contains($ht,'RewriteCond %{REQUEST_URI} ^/dev/images/')&&str_contains($ht,'RewriteRule ^images/(?:[A-Za-z0-9_-]+/)*[A-Za-z0-9][A-Za-z0-9._-]{0,180}\\.(?:jpe?g|png|webp|gif|svg)$ images/template-placeholder.svg [L,NC]'),'DEV ontbrekende images vallen lokaal terug op neutrale placeholder');
laa(!str_contains($ht,'/images/$1 [R=302,L,NE,NC]'),'DEV assetfallback is niet meer afhankelijk van productie-rootassets');
