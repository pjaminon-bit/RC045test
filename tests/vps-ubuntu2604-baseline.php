...
$checks = [
    ...
    [str_contains($contract, "'required_php_modules' => platformPhpRequiredExtensions()") && str_contains($bootstrap, 'platformPhpRequiredExtensions()'), 'production preflight eist alle gebruikte PHP-modules via centrale runtimebron'],
    [str_contains($readiness, 'Ubuntu Server 26.04 LTS') && str_contains($readiness, 'php8.5-fpm') && str_contains($readiness, 'php8.5-pgsql') && str_contains($readiness, 'php8.5-mbstring') && str_contains($readiness, 'php8.5-curl') && str_contains($readiness, 'php8.5-xml') && str_contains($readiness, 'php8.5-gd'), 'readiness bevat volledige Ubuntu 26.04 PHP-packagebaseline'],
    [str_contains($readiness, "openssl|pdo_pgsql|mbstring|curl|dom|gd") && str_contains($readiness, 'DOMDocument') && str_contains($readiness, 'DOMXPath'), 'readiness controleert alle vereiste PHP-modules en DOM-klassen'],
    ...
    [str_contains($phpWorkflow, 'extensions: openssl, pdo_pgsql, mbstring, curl, dom, gd') && str_contains($phpWorkflow, 'grep -Fx mbstring') && str_contains($phpWorkflow, 'grep -Fx curl') && str_contains($phpWorkflow, 'grep -Fx dom') && str_contains($phpWorkflow, 'grep -Fx gd') && str_contains($phpWorkflow, 'DOMDocument') && str_contains($phpWorkflow, 'DOMXPath'), 'PHP 8.5 workflow bewijst volledige module-, GD- en DOM-baseline'],
...
if ($errors !== []) exit(1);
echo "OK: Ubuntu 26.04 / PHP 8.5 productiebaseline is consistent inclusief GD en DOM; expliciete legacy-compatibiliteitstests blijven toegestaan\n";
