...
test('publieke formulieren zijn same-origin en hebben browservalidatie zonder mutatie',async({page})=>{await page.goto(target('/aanmelden.html'),{waitUntil:'domcontentloaded',timeout:45000});await page.waitForTimeout(500);const form=page.locator('#aanmeld-form');expect(await form.count()).toBe(1);expect(await form.locator('[required]').count(),'Aanmeldformulier heeft geen verplichte velden').toBeGreaterThan(0);expect(await form.evaluate(el=>!el.checkValidity()),'Leeg aanmeldformulier moet ongeldig zijn').toBe(true);expect((await form.getAttribute('method')||'get').toLowerCase()).toBe('post');const action=await form.getAttribute('action');expect(action,'Aanmeldformulier mist action').toBeTruthy();const actionUrl=new URL(action,page.url());expect(actionUrl.origin,'Aanmeldformulier mag geen cross-origin action hebben').toBe(baseUrl.origin);expect(actionUrl.pathname).toBe(basePath+'/aanmelden-ontvangst.php');expect((await page.content()).toLowerCase()).not.toContain('formspree.io');});
