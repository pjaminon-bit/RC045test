...
$menuCss=t252txt($root.'/beheer/csp205-index-8e244dd87d1c.css');
...
t252(strpos($menu,'href="csp205-index-8e244dd87d1c.css"')!==false&&strpos($menuCss,'min-height:44px')!==false&&strpos($menuCss,'.links{display:grid;gap:10px}')!==false,'dashboard gebruikt consistent module-ritme en minimale klikhoogte via het geladen CSP-stylesheet');
...
$route=t252txt($root.'/beheer/groep-relaties.php');$routeCss=t252txt($root.'/beheer/csp205-groep-relaties-c82a61085dfe.css');t252($route!=='','groep-relaties beheerroute bestaat');t252(strpos($route,'csrfOk(')!==false,'groep-relaties controleert CSRF');t252(strpos($route,"authHeeftCapability('tasks.manage')")!==false&&strpos($route,"authHeeftCapability('meetings.manage')")!==false&&strpos($route,"authHeeftCapability('events.manage')")!==false,'relatiecategorieën respecteren objectrechten');t252(strpos($route,'if(!$magTaken&&!$magVergaderingen&&!$magEvenementen)')!==false,'lege relatiepagina faalt gesloten zonder objectrechten');t252(strpos($route,'dataSlotOpen()')!==false,'relaties schrijven onder dataslot');t252(strpos($route,'groepenSchrijfDocument')!==false,'relaties gebruiken tenant-private groepenrepository');t252(strpos($route,'href="csp205-groep-relaties-c82a61085dfe.css"')!==false&&strpos($routeCss,'grid-template-columns:repeat(3,minmax(0,1fr))')!==false,'relatiegrid voorkomt overflow door minmax via het geladen CSP-stylesheet');t252(strpos($route,'href="csp205-groep-relaties-c82a61085dfe.css"')!==false&&strpos($routeCss,'select{width:100%;min-width:0}')!==false,'relatieselect is mobiel responsief via het geladen CSP-stylesheet');
