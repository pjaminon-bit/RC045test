...
function siteConfig(): array
{
    static $config = null;
    if ($config === null) {
        $config = require siteProjectRoot() . '/site-config.php';
    }
    return $config;
}
...
function siteModuleVoorPagina(string $pagina): ?string { return siteModuleVoorWaarde('publieke_paginas', $pagina); }
function siteModuleVoorBeheerTab(string $tab): ?string { return siteModuleVoorWaarde('beheer_tabs', $tab); }
function siteModuleVoorBeheerFormulier(string $formulier): ?string { return siteModuleVoorWaarde('beheer_formulieren', $formulier); }
...
function siteScriptPad(): string
{
    $script = str_replace('\\', '/', (string) ($_SERVER['SCRIPT_NAME'] ?? ''));
    return strtolower((string) (parse_url($script, PHP_URL_PATH) ?: $script));
}

function siteHuidigScript(): string
{
    return basename(siteScriptPad());
}

function siteIsBeheerPagina(): bool
{
    $pad = siteScriptPad();
    return basename($pad) === 'beheer.php'
        || preg_match('~(?:^|/)beheer/(?:index\.php)?$~', $pad) === 1;
}
...
function siteBewaakBeheerModulePost(): void
{
    ...
    $_POST['formulier'] = '';
    $GLOBALS['siteModulePostGeblokkeerd'] = ['module' => $module, 'formulier' => $formulier];

    if (function_exists('schrijfLog') && isset($GLOBALS['logBestand'], $GLOBALS['huidigeGebruiker'])) {
        schrijfLog($GLOBALS['logBestand'], (string) $GLOBALS['huidigeGebruiker'], 'module_geblokkeerd', $module . ':' . $formulier);
    }
}
