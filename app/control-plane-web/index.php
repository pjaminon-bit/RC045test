...
$key=(string)$t['tenant_key'];$status=(string)$t['status'];$acties=cp51ToegestaneActies($t);$host=(string)($t['canonical_host']??'');$healthy=($t['healthy']??false)===true;
$redenen=cpOpsTenantAttention($t);$heeftAandacht=$redenen!==[];$statusAge=cpOpsStatusLeeftijd(isset($t['updated_at_utc'])?(string)$t['updated_at_utc']:null);$export=cpOpsExportStatus($t);$transition=$t['transition']??null;
?>
...
<div class="meta">
<div class="<?=$status==='active'&&!$healthy?'bad':''?>"><span>Health</span><strong><?=$healthy?'Gezond':($status==='setup_required'?'Nog niet actief':'Niet gezond / n.v.t.')?></strong><small><?=$status==='active'?'actuele monitoring':'lifecycle-afhankelijk'?></small></div>
<div><span>Laatste status</span><strong><?=h51(tijd51($t['updated_at_utc']??null))?></strong><small><?=h51(leeftijd51($statusAge))?> geleden</small></div>
<div class="<?=$status==='suspended'&&!$export['available']?'bad':''?>"><span>Veilige export</span><strong><?=$export['available']?h51(tijd51($export['created_at_utc'])):'Niet beschikbaar'?></strong><small><?=$export['available']?h51(leeftijd51($export['age_seconds'])).' geleden':'vereist vóór verwijderen'?></small></div>
<div><span>Purge vanaf</span><strong><?=h51(tijd51($t['purge_not_before_utc']??null))?></strong><small><?=$status==='pending_delete'?'24-uurs waarborg':'n.v.t.'?></small></div>
<div><span>Transition</span><strong><?=is_array($transition)?h51((string)($transition['action']??'onbekend')):'Geen'?></strong><small><?=is_array($transition)?'herstelactie beschikbaar':'stabiele lifecycle-state'?></small></div>
</div>
...
<form method="post" autocomplete="off" <?=$actie==='suspend'?'data-confirm-suspend="1" data-tenant-label="'.h51($key).'"':''?>><input type="hidden" name="csrf" value="<?=h51($csrf)?>"><input type="hidden" name="tenant" value="<?=h51($key)?>"><input type="hidden" name="action" value="<?=h51($actie)?>">
...
<button class="btn secondary copy-btn" type="button" data-copy="<?=h51($key)?>">Kopieer key</button>
<?php if($host!==''):?><button class="btn secondary copy-btn" type="button" data-copy="<?=h51($host)?>">Kopieer domein</button><a class="btn" href="https://<?=h51($host)?>/beheer/" target="_blank" rel="noopener noreferrer">Tenantbeheer ↗</a><a class="btn" href="https://<?=h51($host)?>/" target="_blank" rel="noopener noreferrer">Open website ↗</a><?php endif;?>
</div>
</div></section>
<?php endforeach;?>
</div>

<section class="card pending-card"><div class="head"><div><h2>Openstaande aanvragen</h2><div class="host">Alleen pending aanvragen van de huidige operator. De root-only processing-map blijft bewust afgeschermd.</div></div><span class="badge"><?=count($openstaandeActies)?> openstaand</span></div>
<?php if(!$openstaandeActies):?><div class="empty" style="margin-top:12px">Geen eigen aanvragen wachten op verwerking.</div><?php else:?><table class="history-table"><thead><tr><th>Aangevraagd</th><th>Vereniging</th><th>Actie</th><th>Leeftijd</th><th>Request</th></tr></thead><tbody>
<?php foreach($openstaandeActies as $r):?><tr><td><?=h51(tijd51($r['requested_at_utc']))?></td><td><?=h51($r['tenant_key'])?></td><td><?=h51(action51($r['action']))?></td><td class="<?=$r['stale']?'stale':''?>"><?=h51(leeftijd51((int)$r['age_seconds']))?><?=$r['stale']?' · controleer queue':''?></td><td><code><?=h51(substr($r['request_id'],0,8))?></code></td></tr><?php endforeach;?>
...
(() => {
  ...
  document.querySelectorAll('form[data-confirm-suspend="1"]').forEach(form => {
    form.addEventListener('submit', event => {
      const tenant = form.dataset.tenantLabel || 'deze vereniging';
      const akkoord = window.confirm(`Weet je het zeker?\n\n${tenant} wordt uitgeschakeld. De website toont daarna de tijdelijke placeholder en de tenant-runtime/database worden gestopt. Je kunt de vereniging later weer heractiveren.`);
      if (!akkoord) event.preventDefault();
    });
  });
  ...
  document.querySelectorAll('.copy-btn').forEach(button => {
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        window.setTimeout(() => { button.textContent = old; button.classList.remove('copy-ok'); }, 1600);
      } catch (_) {
        window.prompt('Kopieer deze waarde:', value);
      }
    });
  });
})();
