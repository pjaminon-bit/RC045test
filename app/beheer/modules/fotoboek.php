...
function beheerFotoboekStartOutputFilter(): void
{
    ob_start(function ($html) {
        ...
        if (beheerFotoboekMagOpenen()) {
            $html = preg_replace(
                '~<button\s+type="button"\s+class="menu-item"\s+data-tab="fotoboek">.*?</button>~is',
                '<a class="menu-item menu-item-link" style="display:block;text-decoration:none" href="fotoboek.php">* Fotoboek</a>',
                $html,
                1
            ) ?? $html;
        }
        if (stripos($html, '</head>') !== false) {
            $css = '<style id="beheer-fotoboek-legacy-hidden">#tab-fotoboek,[href="#tab-fotoboek"],[href="#fotoboek"],[data-tab-target="fotoboek"]{display:none!important}</style>';
