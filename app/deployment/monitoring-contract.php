...
function monitoring46Plan(array $c):array
{
    ...
    return[
      ...
      'runtime'=>['user'=>$c['runtime_user'],'group'=>$c['runtime_group'],'php_version'=>$php,'php_binary'=>'/usr/bin/php'.$php,'fpm_service'=>'php'.$php.'-fpm.service','fpm_pool'=>$c['pool'],'fpm_socket'=>$c['socket'],'apache_service'=>'apache2.service','postgresql_service'=>'postgresql.service','tenant_https_enabled'=>'/etc/apache2/sites-enabled/'.$c['tenant_https_filename']],
      ...
      'security'=>['no_secrets_in_bundle'=>true,'health_endpoint_discloses_no_tenant_identity'=>true,'health_endpoint_discloses_no_failure_detail'=>true,'monitoring_paths_outside_document_root'=>true,'alert_adapter_outside_git'=>true,'raw_request_headers_forbidden_in_platform_access_log'=>true,'fpm_service_log_uses_system_journal'=>true,'health_runner_uses_exact_tenant_php'=>true],
    ];
}
...
function monitoring46TrustedRoot():string{$root=realpath(dirname(__DIR__,2));if($root===false||!is_dir($root))throw new RuntimeException('Monitoring trusted source-root kon niet fysiek worden opgelost.');$checker=$root.'/bin/check-vps-health.php';if(!is_file($checker)||is_link($checker))throw new RuntimeException('Monitoring trusted healthchecker ontbreekt of is een symlink.');return runtime41NormPad($root);}
function monitoring46SystemdService(array $p):string{$root=monitoring46TrustedRoot();$php=(string)$p['runtime']['php_binary'];if(!preg_match('#^/usr/bin/php[0-9]{1,2}\.[0-9]{1,2}$#D',$php))throw new RuntimeException('Monitoring systemd-service mist exact tenant-PHP pad.');return implode("\n",['[Unit]','Description=Verenigingsplatform healthcheck '.$p['tenant_key'],'After=network-online.target apache2.service '.$p['runtime']['fpm_service'].' postgresql.service','','[Service]','Type=oneshot','ExecStart='.$php.' '.$root.'/bin/check-vps-health.php --monitoring-plan='.$p['bundle']['plan_file'].' --probe --write-status --alert','NoNewPrivileges=true','PrivateTmp=true','ProtectHome=true','ProtectSystem=strict','ReadWritePaths='.$p['logging']['app_dir'].' /var/lib/verenigingsplatform/monitoring','','']);}
