...
function monitoring46Plan(array $c):array
{
    ...
    return[
      ...
      'security'=>['no_secrets_in_bundle'=>true,'health_endpoint_discloses_no_tenant_identity'=>true,'health_endpoint_discloses_no_failure_detail'=>true,'monitoring_paths_outside_document_root'=>true,'alert_adapter_outside_git'=>true,'raw_request_headers_forbidden_in_platform_access_log'=>true,'fpm_service_log_uses_system_journal'=>true,'health_runner_uses_root_owned_host_launcher'=>true],
    ];
}
...
function monitoring46SystemdService(array $p):string{$launcher=(string)($p['systemd']['host_launcher']??'');if(!hash_equals(monitoring46HostLauncher(),$launcher))throw new RuntimeException('Monitoring systemd-service mist de vaste root-owned host-launcher.');return implode("\n",['[Unit]','Description=Verenigingsplatform healthcheck '.$p['tenant_key'],'After=network-online.target apache2.service '.$p['runtime']['fpm_service'].' postgresql.service','','[Service]','Type=oneshot','ExecStart='.$launcher.' health --monitoring-plan='.$p['bundle']['plan_file'],'NoNewPrivileges=true','PrivateTmp=true','ProtectHome=true','ProtectSystem=strict','ReadWritePaths='.$p['logging']['app_dir'].' /var/lib/verenigingsplatform/monitoring','','']);}
