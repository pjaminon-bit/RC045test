...
function ledenServiceVindVoorAccount(string $userId,string $gebruikersnaam=''): ?array{$userId=trim($userId);$gebruikersnaam=trim($gebruikersnaam);foreach((array)(ledenServiceLees()['leden']??[]) as $lid){if(!is_array($lid)||!empty($lid['gearchiveerd_op']))continue;$uid=trim((string)($lid['user_id']??''));if($userId!==''&&$uid!==''&&hash_equals($uid,$userId))return$lid;if($gebruikersnaam!==''&&strcasecmp(trim((string)($lid['beheer_account']??'')),$gebruikersnaam)===0)return$lid;}return null;}
...
function ledenServiceDefinitiefVerwijder(array &$data,string $lidId): ?array{$gevonden=null;$over=[];foreach((array)($data['leden']??[]) as $lid){if(is_array($lid)&&($lid['id']??'')===$lidId){$gevonden=$lid;continue;}$over[]=$lid;}if($gevonden===null||empty($gevonden['gearchiveerd_op']))return null;if(isset($data['commissies'])&&is_array($data['commissies']))ledenServicePurgeId($data['commissies'],$lidId);if(!ledenServiceVerwijderRelaties($lidId,(string)($gevonden['aanmelding_id']??'')))return null;$data['leden']=$over;return$gevonden;}
function ledenServiceType(array $lid): ?array{$id=trim((string)($lid['lidmaatschap_type']??''));if($id!==''){$t=lidmaatschapTypeOpId($id);if($t)return$t;}$jaar=(int)date('Y');$leeftijd=ledenLeeftijd((string)($lid['geboortedatum']??''),$jaar.'-01-01');return lidmaatschapTypeVoorLeeftijd($leeftijd);}
