...
function groepenNormaliseerRollen(array $rollen): array{$uit=[];$gezien=[];foreach($rollen as $r){if(!is_array($r))continue;$id=groepenId($r['id']??'');$naam=groepenKort($r['naam']??'',60);if($id===''||$naam===''||isset($gezien[$id]))continue;$gezien[$id]=true;$uit[]=['id'=>$id,'naam'=>$naam,'actief'=>($r['actief']??true)!==false];}return$uit?:groepenLeeg()['rollen'];}
...
function groepenWerkLedenBij(array $bestaand,array $geselecteerd,array $rollenPerLid): array{$vandaag=date('Y-m-d');$uit=[];$actief=[];foreach((array)$bestaand as $m){if(!is_array($m))continue;if(($m['tot']??'')==='')$actief[(string)($m['lid_id']??'')]=count($uit);$uit[]=$m;}$set=array_fill_keys(array_values(array_unique(array_filter(array_map('strval',$geselecteerd)))),true);foreach($actief as $lidId=>$idx){if(!isset($set[$lidId]))$uit[$idx]['tot']=$vandaag;else{$uit[$idx]['rollen']=(array)($rollenPerLid[$lidId]??['lid']);unset($set[$lidId]);}}foreach(array_keys($set) as $lidId)$uit[]=['lid_id'=>$lidId,'rollen'=>(array)($rollenPerLid[$lidId]??['lid']),'sinds'=>$vandaag,'tot'=>''];return$uit;}
function groepenWerkLedenBijBeheer(array $bestaand,array $geselecteerd,array $rollenPerLid,array $leden): array
{
    $lidMap=[];
    foreach($leden as $lid){if(!is_array($lid))continue;$lidId=trim((string)($lid['id']??''));if($lidId===''||strlen($lidId)>80)continue;$lidMap[$lidId]=$lid;}
    $effectief=[];$rollen=[];
    foreach($geselecteerd as $lidId){$lidId=trim((string)$lidId);if($lidId===''||strlen($lidId)>80||!isset($lidMap[$lidId])||!empty($lidMap[$lidId]['gearchiveerd_op']))continue;$effectief[$lidId]=true;$rollen[$lidId]=(array)($rollenPerLid[$lidId]??['lid']);}
    foreach($bestaand as $m){if(!is_array($m)||($m['tot']??'')!=='')continue;$lidId=trim((string)($m['lid_id']??''));if($lidId===''||!isset($lidMap[$lidId])||empty($lidMap[$lidId]['gearchiveerd_op']))continue;$effectief[$lidId]=true;$rollen[$lidId]=(array)($m['rollen']??['lid']);}
    return groepenWerkLedenBij($bestaand,array_keys($effectief),$rollen);
}
function groepenWerkLidBij(array $bestaand,string $lidId,bool $actief,array $rollen=[]): array{$lidId=groepenKort($lidId,80);if($lidId==='')return$bestaand;$vandaag=date('Y-m-d');$uit=[];$actiefIndex=null;foreach((array)$bestaand as $m){if(!is_array($m))continue;if(($m['lid_id']??'')===$lidId&&($m['tot']??'')===''&&$actiefIndex===null)$actiefIndex=count($uit);$uit[]=$m;}$rollen=array_values(array_unique(array_filter(array_map('groepenId',$rollen))));if(!$rollen)$rollen=['lid'];if($actief){if($actiefIndex===null)$uit[]=['lid_id'=>$lidId,'rollen'=>$rollen,'sinds'=>$vandaag,'tot'=>''];else$uit[$actiefIndex]['rollen']=$rollen;}elseif($actiefIndex!==null)$uit[$actiefIndex]['tot']=$vandaag;return$uit;}
...
function groepenVind(array $doc,string $groepId): ?array{$groepId=groepenId($groepId);foreach((array)($doc['groepen']??[]) as $g)if(is_array($g)&&($g['id']??'')===$groepId)return$g;return null;}
function groepenRelatiesVoorGroep(array $doc,string $groepId): array{$groepId=groepenId($groepId);$leeg=[];foreach(groepenRelatieTypes() as $type=>$label)$leeg[$type]=[];$set=$doc['relaties'][$groepId]??[];foreach($leeg as $type=>$_)$leeg[$type]=groepenNormaliseerRelatieIds($set[$type]??[]);return$leeg;}
function groepenRelatiesWerkBij(array &$doc,string $groepId,array $relaties): bool{$groepId=groepenId($groepId);if(!groepenVind($doc,$groepId))return false;if(!isset($doc['relaties'])||!is_array($doc['relaties']))$doc['relaties']=[];$rij=[];foreach(groepenRelatieTypes() as $type=>$label)$rij[$type]=groepenNormaliseerRelatieIds($relaties[$type]??[]);$doc['relaties'][$groepId]=$rij;return true;}
function groepenRelatiesObjectGroepen(array $doc,string $type,string $objectId): array{$objectId=groepenKort($objectId,80);if(!isset(groepenRelatieTypes()[$type])||$objectId==='')return[];$uit=[];foreach((array)($doc['relaties']??[]) as $groepId=>$set)if(in_array($objectId,(array)($set[$type]??[]),true)){$groep=groepenVind($doc,(string)$groepId);if($groep)$uit[]=$groep;}return$uit;}
