...
sudo apt install -y \
  ...
  php8.5-cli php8.5-fpm php8.5-pgsql php8.5-mbstring php8.5-curl php8.5-xml php8.5-gd \
...
- PHP-modules `openssl`, `pdo_pgsql`, `mbstring`, `curl`, `dom` en `gd`, plus beschikbare `DOMDocument`- en `DOMXPath`-klassen;
...
uname -a
php8.5 -v
php8.5 -m | grep -E '^(openssl|pdo_pgsql|mbstring|curl|dom|gd)$'
