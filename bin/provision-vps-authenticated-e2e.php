...
$opt = getopt('', ['config:','expected-tenant:','expected-site:','admin-user:','member-user:','password-stdin','check','apply','help']);
if (isset($opt['help'])) {
    echo "Gebruik:\n";
    echo "  php bin/provision-vps-authenticated-e2e.php --config=/srv/verenigingen/test/config.php --expected-tenant=test --expected-site=https://test.vps.holox.nl --admin-user=vps-e2e-admin --member-user=vps-e2e-member --check\n";
    echo "  printf '%s\\n' \"\$E2E_PASSWORD\" | php bin/provision-vps-authenticated-e2e.php ... --password-stdin --apply\n";
    exit(0);
}
$check = isset($opt['check']); $apply = isset($opt['apply']);
if ($check === $apply) { fwrite(STDERR, "FOUT: kies exact één van --check of --apply.\n"); exit(2); }
if ($apply && !isset($opt['password-stdin'])) { fwrite(STDERR, "FOUT: --apply vereist --password-stdin.\n"); exit(2); }

$configPad=trim((string)($opt['config']??'')); $expectedTenant=trim((string)($opt['expected-tenant']??''));
$expectedSite=rtrim(trim((string)($opt['expected-site']??'')),'/'); $adminUser=trim((string)($opt['admin-user']??'')); $memberUser=trim((string)($opt['member-user']??''));
...
function e2e510Stop(string $message,int $code=1):void{fwrite(STDERR,'FOUT: '.$message."\n");exit($code);}
function e2e510AuthLees(string $pad):array{
...
try{
    $config=require $root.'/site-config.php';if(!is_array($config))throw new RuntimeException('site-config levert geen array.');
    $tenant=tenantRuntimeVeiligeSleutel((string)($config['vereniging']['sleutel']??''));$site=rtrim((string)($config['vereniging']['site_url']??''),'/');
    ...
    e2e510Username($adminUser);e2e510Username($memberUser);if(strcasecmp($adminUser,$memberUser)===0)throw new RuntimeException('E2E-admin en E2E-lid moeten verschillende accounts zijn.');
    ...
    $privateRoot=tenantRuntimePrivateRoot($config);if($privateRoot===null||!is_dir($privateRoot)||is_link($privateRoot)||!is_readable($privateRoot)||!is_writable($privateRoot))throw new RuntimeException('Tenant private_root is niet veilig leesbaar/schrijfbaar.');
    authStorageValideerExterneMaster($privateRoot);$authPad=$privateRoot.'/auth/users.json';$authBackup=$privateRoot.'/backups/auth';$users=e2e510AuthLees($authPad);
    $probeHash=password_hash(bin2hex(random_bytes(24)),PASSWORD_DEFAULT);if(!is_string($probeHash))throw new RuntimeException('Password hash probe faalde.');e2e510MergeAuthUsers($users,$tenant,$adminUser,$memberUser,$probeHash);

    $leden=e2e510MergeLeden(repoLedenLees(),$tenant,$memberUser);
    $contributies=e2e510MergeContributies(contributiesLees(),$tenant);
    $groepen=e2e510MergeGroepen(groepenLeesDocument(),$tenant);
    $vergaderingen=e2e510MergeVergaderingen(repoVergaderingenLees(),$tenant);
    $taken=e2e510MergeTaken(repoTakenLees(),$tenant);
    if($check){echo 'E2E CHECK OK  tenant='.$tenant.' storage=pdo fixture='.e2e510Marker()."\n";exit(0);}

    $password=stream_get_contents(STDIN);$password=is_string($password)?rtrim($password,"\r\n"):'';
    if(strlen($password)<24||strlen($password)>256||str_contains($password,"\0"))throw new RuntimeException('E2E-wachtwoord via stdin moet 24-256 tekens bevatten.');
    $hash=password_hash($password,PASSWORD_DEFAULT);$password=str_repeat("\0",strlen($password));unset($password);
    ...
    $authBestond=is_file($authPad)&&!is_link($authPad);$authRaw=$authBestond?file_get_contents($authPad):null;if($authBestond&&!is_string($authRaw))throw new RuntimeException('Originele authstore kon niet worden gesnapshot.');

    e2e510AuthSchrijf($authPad,$newUsers,$authBackup);
    try{
        privateStoreTransactie(function()use($leden,$contributies,$groepen,$vergaderingen,$taken,$tenant):void{
            ...
            $ids=e2e510Ids($tenant);$lidOk=false;foreach((array)(repoLedenLees()['leden']??[])as$lid)if(is_array($lid)&&($lid['id']??'')===$ids['member']&&($lid['user_id']??'')===$ids['member_user'])$lidOk=true;if(!$lidOk)throw new RuntimeException('Ledenfixture kon niet na write worden geverifieerd.');
            $groepOk=false;foreach((array)(groepenLeesDocument()['groepen']??[])as$groep)if(is_array($groep)&&($groep['id']??'')===$ids['group'])$groepOk=true;if(!$groepOk)throw new RuntimeException('Groepsfixture kon niet na write worden geverifieerd.');
        });
    }catch(Throwable$e){e2e510AuthHerstel($authPad,$authBestond,$authRaw);throw$e;}

    $verifyUsers=e2e510AuthLees($authPad);$adminOk=$memberOk=false;foreach($verifyUsers as$record){if(!is_array($record))continue;if(e2e510FixtureRecord($record,$tenant,'admin')&&strcasecmp((string)($record['gebruikersnaam']??''),$adminUser)===0)$adminOk=true;if(e2e510FixtureRecord($record,$tenant,'member')&&strcasecmp((string)($record['gebruikersnaam']??''),$memberUser)===0)$memberOk=true;}
    if(!$adminOk||!$memberOk)throw new RuntimeException('E2E-authaccounts konden niet worden nageverifieerd.');
    echo 'E2E APPLY OK  tenant='.$tenant.' accounts=2 linked_member=1 fixture='.e2e510Marker()."\n";
}catch(Throwable$e){e2e510Stop($e->getMessage());}
