...
if($mode==='bootstrap'){
    ...
    $entry=apply47Stage($plan,$manifest);apply47Switch($plan,$entry);apply47AtomicJson((string)$plan['paths']['state'],apply47State($entry,null,null,0,true));apply47Event($plan,'bootstrap',['mode'=>'bootstrap','to_commit'=>$entry['commit'],'result'=>'ok','tenant_count'=>0]);echo'BOOTSTRAP OK  commit='.$entry['commit']."\n";exit(0);
}
if($mode==='deploy'){
...
if(!apply47FpmReload($tenants)||!apply47Health((string)$target['path'],$tenants,false)){
    apply47Switch($plan,$state['active']);$restored=apply47FpmReload($tenants);apply47AtomicJson((string)$plan['paths']['state'],apply47State($state['active'],$state['previous'],null,count($tenants),false));apply47Event($plan,'rollback_failed',['mode'=>'rollback','from_commit'=>$state['active']['commit'],'to_commit'=>$target['commit'],'result'=>$restored?'current-restored':'reload-failed','tenant_count'=>count($tenants)]);apply47Stop('Rollbackdoel werd niet gezond; oorspronkelijke current is teruggezet.',3);
}
