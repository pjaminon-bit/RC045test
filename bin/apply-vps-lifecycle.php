...
function apply48Lock(array $p)
{
    ...
    $h=@fopen($lock,'c');if(!is_resource($h))throw new RuntimeException('Lifecycle-lock kon niet worden geopend.');@chown($lock,0);@chgrp($lock,0);@chmod($lock,0600);
...
function apply48Export(array$p,array$s):array
{
    ...
    try{$dbDump=$stage.'/database.dump';[$c,,$e]=apply48Run(['runuser','-u','postgres','--','pg_dump','-Fc','-d',(string)$p['database']['database']],null,$dbDump);if($c!==0)throw new RuntimeException('PostgreSQL export faalt: '.$e);@chown($dbDump,0);@chgrp($dbDump,0);@chmod($dbDump,0600);
        $fs=$stage.'/tenant-files.tar.gz';[$c,,$e]=apply48Run(['/usr/bin/tar','--create','--gzip','--file='.$fs,'--directory='.dirname((string)$p['filesystem']['tenant_root']),basename((string)$p['filesystem']['tenant_root'])]);if($c!==0)throw new RuntimeException('Tenant filesystem-export faalt: '.$e);@chown($fs,0);@chgrp($fs,0);@chmod($fs,0600);
