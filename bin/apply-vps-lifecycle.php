...
function apply48Run(array $cmd, ?string $stdin = null, ?string $stdoutFile = null): array
{
    ...
    if ($stdin !== null) fwrite($x[0], $stdin); fclose($x[0]);
    $out = $stdoutFile === null ? stream_get_contents($x[1]) : ''; if ($stdoutFile === null) fclose($x[1]);
...
function apply48SafeDir(string $p, int $mode = 0750, string|int $group = 0): void
{
    ...
    if (runtime41SymlinkInPad($p) !== null || !@chown($p,0) || !@chgrp($p,$group) || !@chmod($p,$mode)) throw new RuntimeException('Onveilige lifecyclemap: ' . $p);
}
...
function apply48Audit(array $p,string $action,string $result,string $before,?string $after=null,array $extra=[]):void
{
    ...
    $j=json_encode($r,JSON_UNESCAPED_SLASHES);if(!is_string($j)||@file_put_contents($f,$j."\n",FILE_APPEND|LOCK_EX)===false)throw new RuntimeException('Lifecycle audit kon niet worden geschreven.');@chown($f,0);@chgrp($f,'adm');@chmod($f,0640);
}
...
function apply48ApacheTestReload(array $p):void{[$c,$o,$e]=apply48Run([(string)$p['apache']['control_binary'],'configtest']);if($c!==0)throw new RuntimeException('Apache configtest faalt: '.trim($o."\n".$e));[$c,,$e]=apply48Run(['systemctl','reload','apache2']);if($c!==0)throw new RuntimeException('Apache reload faalt: '.$e);}
function apply48ApacheAan(array $p):void
{
    apply48ExactFile((string)$p['apache']['tenant_http_available'],(string)$p['apache']['tenant_http_bundle'],'HTTP-vhost');apply48ExactFile((string)$p['apache']['tenant_https_available'],(string)$p['apache']['tenant_https_bundle'],'HTTPS-vhost');
    ...
    try{foreach($paren as[$d,$l]){$bestond=apply48LinkExact($l,$d);$nieuw[$l]=!$bestond;if(!$bestond&&!@symlink($d,$l))throw new RuntimeException('Tenant Apache-link kon niet worden gemaakt.');}apply48ApacheTestReload($p);}catch(Throwable$e){foreach($nieuw as$l=>$n)if($n&&is_link($l))@unlink($l);try{apply48ApacheTestReload($p);}catch(Throwable$ignored){}throw$e;}
}
function apply48ApacheUit(array $p):void
{
    $weg=[];foreach([[$p['apache']['tenant_http_enabled'],$p['apache']['tenant_http_available']],[$p['apache']['tenant_https_enabled'],$p['apache']['tenant_https_available']]]as[$l,$d]){if(is_link($l)){if(realpath($l)!==realpath($d))throw new RuntimeException('Onverwachte tenant Apache-link: '.$l);$weg[]=[(string)$l,(string)$d];}elseif(file_exists($l))throw new RuntimeException('Tenant Apache enabled-doel is geen symlink.');}
    foreach($weg as[$l])@unlink($l);try{apply48ApacheTestReload($p);}catch(Throwable$e){foreach($weg as[$l,$d])if(!file_exists($l)&&!is_link($l))@symlink($d,$l);try{apply48ApacheTestReload($p);}catch(Throwable$ignored){}throw$e;}
}
...
function apply48ProcessenStil(array$p):bool{[$c,$o]=apply48Run(['pgrep','-u',(string)$p['runtime']['user']]);return$c===1||($c===0&&trim($o)==='');}
...
function apply48SuspendedResources(array$p):void{apply48ApacheUit($p);apply48DbUit($p);apply48TimerUit($p);apply48FpmUit($p);}
...
function apply48Export(array$p,array$s):array
{
    ...
    $root=(string)$p['filesystem']['export_root'];apply48SafeDir(dirname($root),0700);apply48SafeDir($root,0700);$stamp=gmdate('Ymd_His');$stage=$root.'/.stage-'.$stamp.'-'.bin2hex(random_bytes(4));apply48SafeDir($stage,0700);
    try{$dbDump=$stage.'/database.dump';[$c,,$e]=apply48Run(['runuser','-u','postgres','--','pg_dump','-Fc','-d',(string)$p['database']['database']],null,$dbDump);if($c!==0)throw new RuntimeException('PostgreSQL export faalt: '.$e);@chown($dbDump,0);@chgrp($dbDump,0);@chmod($dbDump,0600);
        $fs=$stage.'/tenant-files.tar.gz';[$c,,$e]=apply48Run(['/usr/bin/tar','--create','--gzip','--file='.$fs,'--directory='.dirname((string)$p['filesystem']['tenant_root']),basename((string)$p['filesystem']['tenant_root'])]);if($c!==0)throw new RuntimeException('Tenant filesystem-export faalt: '.$e);@chmod($fs,0600);
        $manifest=['schema'=>1,'phase'=>'4.8-export','tenant_key'=>$p['tenant_key'],'created_at_utc'=>apply48Utc(),'state_generation'=>(int)$s['generation'],'database_sha256'=>hash_file('sha256',$dbDump),'tenant_files_sha256'=>hash_file('sha256',$fs)];$mj=lifecycle48Json($manifest);apply48Write($stage.'/export-manifest.json',$mj,0600);
        $pkg=$root.'/'.$stamp.'-tenant-export.tar.gz';if(file_exists($pkg)||is_link($pkg))throw new RuntimeException('Exportdoel bestaat onverwacht al.');[$c,,$e]=apply48Run(['/usr/bin/tar','--create','--gzip','--file='.$pkg,'--directory='.$stage,'database.dump','tenant-files.tar.gz','export-manifest.json']);if($c!==0)throw new RuntimeException('Exportpakket kon niet worden gemaakt: '.$e);@chown($pkg,0);@chgrp($pkg,0);@chmod($pkg,0600);$sha=hash_file('sha256',$pkg);if(!is_string($sha))throw new RuntimeException('Exportchecksum kon niet worden berekend.');return['path'=>$pkg,'sha256'=>$sha,'created_at_utc'=>$manifest['created_at_utc'],'state_generation'=>(int)$s['generation']];
    }finally{apply48Rm($stage);}
}
...
{
    apply48TimerUit($p);if(is_link((string)$p['apache']['tenant_http_enabled'])||is_link((string)$p['apache']['tenant_https_enabled']))apply48ApacheUit($p);
    ...
    foreach([(string)$p['monitoring']['timer_file'],(string)$p['monitoring']['service_file'],(string)$p['monitoring']['tenant_logrotate'],(string)$p['monitoring']['health_status'],(string)$p['monitoring']['alert_state']]as$f){if(is_link($f))throw new RuntimeException('Onverwachte symlink in tenant servicebestand: '.$f);if(is_file($f))@unlink($f);}apply48Run(['systemctl','daemon-reload']);
    foreach([(string)$p['apache']['tenant_http_available'],(string)$p['apache']['tenant_https_available'],(string)$p['apache']['routing_fragment']]as$f){if(is_link($f))throw new RuntimeException('Onverwachte symlink in Apache tenantartifact.');if(is_file($f))@unlink($f);}apply48ApacheTestReload($p);
    ...
    $acme=(string)$p['tls']['acme_webroot'];if(file_exists($acme)){if(runtime41SymlinkInPad($acme)!==null)throw new RuntimeException('ACME webroot is onveilig.');apply48Rm($acme);}
    $u=(string)$p['runtime']['user'];if(!apply48ProcessenStil($p))throw new RuntimeException('Tenant Linux-user heeft nog processen vóór userdel.');[$c,,$e]=apply48Run(['userdel',$u]);if($c!==0&&function_exists('posix_getpwnam')&&is_array(@posix_getpwnam($u)))throw new RuntimeException('Tenant system user kon niet worden verwijderd: '.$e);[$c,,$e]=apply48Run(['groupdel',(string)$p['runtime']['group']]);if($c!==0&&function_exists('posix_getgrnam')&&is_array(@posix_getgrnam((string)$p['runtime']['group'])))throw new RuntimeException('Tenant system group kon niet worden verwijderd: '.$e);
}
...
$o=getopt('',['plan::','check','status','adopt-active','suspend','activate','recover','export','delete','cancel-delete','purge','recover-purge','tenant::','confirm-tenant::','confirm-export-sha::','confirm-purge::','help']);if(isset($o['help'])){echo"Gebruik: php bin/apply-vps-lifecycle.php --plan=... --check | sudo php ... --adopt-active|--suspend|--activate|--recover|--export|--delete|--cancel-delete|--purge\nDefinitieve purge vereist --confirm-tenant=<key> --confirm-export-sha=<sha256> --confirm-purge=VERWIJDER-DEFINITIEF.\nRecovery na crash tijdens laatste dataverwijdering: sudo php ... --recover-purge --tenant=<key>.\n";exit(0);}
...
try{
    if($actie==='recover-purge'){
        apply48Root();$tenant=trim((string)($o['tenant']??''));if(!runtime41CanoniekeTenantKey($tenant))throw new RuntimeException('--tenant is verplicht en moet canoniek zijn.');$snap='/var/lib/verenigingsplatform/lifecycle/plans/'.$tenant.'.json';$tomb='/var/lib/verenigingsplatform/lifecycle/tombstones/'.$tenant.'.json';if(runtime41SymlinkInPad($snap)!==null||!is_file($snap)||is_link($tomb)||!is_file($tomb))throw new RuntimeException('Recovery snapshot/tombstone ontbreekt of is onveilig.');$pr=json_decode((string)file_get_contents($snap),true);$tb=json_decode((string)file_get_contents($tomb),true);if(!is_array($pr)||($pr['phase']??'')!=='4.8'||!hash_equals($tenant,(string)($pr['tenant_key']??''))||!is_array($tb)||($tb['status']??'')!=='data_delete'||!hash_equals($tenant,(string)($tb['tenant_key']??'')))throw new RuntimeException('Recovery metadata is niet geldig voor data-delete.');$root=(string)$pr['filesystem']['tenant_root'];if(file_exists($root)){apply48TreeGeenSymlinks($root);apply48Rm($root);}apply48Tombstone($pr,array_merge($tb,['status'=>'deleted','completed_at_utc'=>apply48Utc()]));@unlink((string)$pr['filesystem']['state_file']);@unlink($snap);echo'RECOVER PURGE OK tenant='.$tenant."\n";exit(0);
    }
    $planPad=trim((string)($o['plan']??''));if($planPad==='')throw new RuntimeException('--plan is verplicht.');$ctx=lifecycle48PlanLeesEnValideer($planPad);$p=$ctx['plan'];if($actie==='check'){echo'CHECK OK tenant='.$p['tenant_key']."\n";exit(0);}apply48Root();if(is_file((string)$p['filesystem']['tombstone_file']))throw new RuntimeException('Tenant heeft een lifecycle-tombstone; hergebruik/activatie wordt fail-closed geweigerd.');$raw=@file_get_contents($ctx['path']);if(!is_string($raw))throw new RuntimeException('Lifecycleplan kon niet voor rootsnapshot worden gelezen.');apply48BasisMappen($p,$raw);$state=apply48StateLees($p,true);$before=$state['status']??'unmanaged';
    if($actie==='status'){echo lifecycle48Json(['tenant_key'=>$p['tenant_key'],'status'=>$before,'state'=>$state]);exit(0);}
