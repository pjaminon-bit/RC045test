...
try{$c=cpeConfig(trim((string)($o['config']??'')));cpeDir(dirname($c['executor_lock']),0755);$lh=@fopen($c['executor_lock'],'c');if(!is_resource($lh)||!flock($lh,LOCK_EX|LOCK_NB))throw new RuntimeException('Er draait al een control-plane executor.');@chown($c['executor_lock'],0);@chgrp($c['executor_lock'],0);@chmod($c['executor_lock'],0600);
