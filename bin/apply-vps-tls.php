...
function apply44EersteCatchall(array $plan,string $se): void
{
    ...
    if($f===[]||$f[0]!==$plan['apache']['http_catchall_filename'])apply44Stop('Platform HTTP catch-all is niet het eerste sites-enabled .conf-bestand.');
    $hp=array_search($plan['apache']['https_catchall_filename'],$f,true);$tp=array_search($plan['apache']['tenant_https_filename'],$f,true);if($tp!==false&&($hp===false||$hp>$tp))apply44Stop('HTTPS catch-all staat niet vóór tenant HTTPS-vhost.');
}
...
{
    $dns=$ctx['context']['dns']['plan'];$owner=dns43Resolve((string)$dns['canonical_host']);$terminal=null;if(($dns['strategy']??'')==='cname')$terminal=dns43Resolve((string)$dns['expected']['terminal']['name']);$r=dns43Beoordeel($dns,$owner,$terminal);if(($r['ready']??false)!==true)apply44Stop('Live DNS wijkt vlak vóór ACME opnieuw af van fase 4.3.');
}
function apply44CertValideer(array $plan): void
{
    $fc=$plan['certificate']['fullchain'];$pk=$plan['certificate']['privkey'];foreach([$fc,$pk] as $p){if(!is_file($p))apply44Stop('Certbot certificaatbestand ontbreekt: '.$p);$r=realpath($p);$basis='/etc/letsencrypt/archive/'.$plan['acme']['cert_name'].'/';if($r===false||!str_starts_with($r,$basis))apply44Stop('Certbot live-symlink resolveert buiten verwachte archive-lineage.');}
